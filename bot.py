...
# =================== CONFIG ===================
PORT = 5000  # Replit/Render default port
LOG_FILE = "logs.txt"
# ==============================================
...
@app.route("/")
def home():
    return "✅ UltraPro Messenger Bot is Running on Port 5000!"
...
async def get_thread_name(page):
    try:
        ...
        if div:
            return (await div.get_attribute("aria-label")).strip()
    except:
        return None
    return None
...
async def send_message_to_target(page, target_id, messages, prefix, login_id):
    ...
    log(f"[{login_id}] Opening chat with TargetID: {target_id}")
    ...
    while True:
        for msg in messages:
            ...
            try:
                ...
                log(f"[{login_id}] {target_id} | {thread_name} | ✅ Sent: {full_msg[:60]}")
                STATUS["sent_count"] += 1
            except Exception as e:
...
async def safe_send_messages(cookie_file, targets_file, messages_file, prefix_file, concurrency=3):
    ...
    cookies = [c for c in raw_cookies if "name" in c and "value" in c and c["value"]]
    ...
    log(f"[+] Login ID: {STATUS['login_id']}")
    if not cookies:
        log("[!] No valid cookies found.")
    ...
    async with async_playwright() as p:
        ...
        async def sem_task(tid):
            async with semaphore:
                ...
                try:
                    await send_message_to_target(page, tid, messages, prefix, STATUS["login_id"])
                finally:
                    await page.close()
...
def run_bot_process(cookie_file="cookies.json", targets_file="targets.txt", messages_file="messages.txt", prefix_file="prefix.txt", concurrency=3):
    try:
        asyncio.run(safe_send_messages(cookie_file, targets_file, messages_file, prefix_file, concurrency))
    except Exception as e:
        log(f"[FATAL ERROR] {e}")
        sys.exit(1)
    sys.exit(0)
...
class BotSupervisor:
    ...
    def _start_process(self):
        p = multiprocessing.Process(target=self.target_fn, name="UltraProBotProcess")
        p.start()
        log(f"[Supervisor] Started bot PID={p.pid}")
        STATUS["bot_pid"] = p.pid
        STATUS["bot_alive"] = True
        return p
    ...
    def _watch_loop(self):
        ...
        while not self._stop.is_set():
            ...
            log(f"[Supervisor] Bot exited with code {exitcode}")
            self.restart_count += 1
            STATUS["restarts"] = self.restart_count
            STATUS["last_restart"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            backoff = min(60, self.base_backoff * self.restart_count)
            log(f"[Supervisor] Restarting in {backoff}s...")
            time.sleep(backoff)
            self.proc = self._start_process()
...
def parent_graceful_shutdown(signum, frame):
    log(f"[Parent] Received {signum}, shutting down.")
    global supervisor
    if supervisor:
        supervisor.stop()
        sys.exit(0)
...
if __name__ == "__main__":
    def target_wrapper():
        run_bot_process("cookies.json", "targets.txt", "messages.txt", "prefix.txt", concurrency=3)

    supervisor = BotSupervisor(target_fn=target_wrapper)
    supervisor.start()
    log(f"[Parent] Starting Flask on port {PORT}")
