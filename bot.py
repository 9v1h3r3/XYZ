...
@app.route("/")
def home(): return "✅ Messenger Bot (E2EE) Running!"
@app.route("/status")
...
async def safe_send_messages(cookie_file, targets_file, messages_file, prefix_file, concurrency=3):
    ...
    cookies=[{"name":c["key"],"value":c["value"],"domain":c["domain"],"path":c.get("path","/")} for c in raw_cookies]
...
class BotSupervisor:
    ...
    def _start_process(self):
        p=multiprocessing.Process(target=self.target_fn,name="E2EEBotProcess"); p.start()
