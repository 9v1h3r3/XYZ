...
def status():
    return jsonify({
        ...
        "last_target": STATUS["last_target"],
        "thread_name": STATUS["last_thread_name"],
...
def run_bot_process(cookie_file="cookies.json", targets_file="targets.txt", messages_file="messages.txt", prefix_file="prefix.txt", concurrency=1):
    try:
...
class BotSupervisor:
    ...
    def _start_process(self):
        p = multiprocessing.Process(target=self.target_fn, name="MessengerBotProcess")
        p.start()
    ...
    def _watch_loop(self):
        ...
        while not self._stop.is_set():
            ...
            log(f"[supervisor] Bot exited code={exitcode}")
...
if __name__ == "__main__":
    def target_wrapper():
        run_bot_process("cookies.json", "targets.txt", "messages.txt", "prefix.txt", concurrency=1)

    supervisor = BotSupervisor(target_fn=target_wrapper)
    supervisor.start()

    log("[parent] Starting Flask on 0.0.0.0:5000")
